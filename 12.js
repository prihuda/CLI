...
cli
  .version('1.0.0')
  .command('movies', 'Get all information about new movies in theaters for today from CGV website')
	  ...
	  http.get(opt, function(res) {
		  ...
		  res.on("end", function() {
			  ...
			  $('div .schedule-title').map(function(i, li) {
				  let movie = {};
				  movie.title = $(li).find('a').text().trim();
				  let opt = {
					  host: 'www.cgv.id',
				  ...
				  http.get(opt, function(res) {
					  ...
					  res.on("end", function() {
						  ...
						  $1('div [class=\"movie-add-info left\"]').find('li').map(function(i, li) {
							  let text = $1(li).text();
						  ...
						  movie.trailer = $1('div [class=trailer-btn-wrapper]').find('img').attr('onclick').split(/[\('\)]/)[2];
						  movie.synopsis = $1('div [class=\"movie-synopsis right\"]').text().split('\n')[1].trim();
